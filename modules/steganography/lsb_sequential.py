...
def embed_file_sequential(image: np.ndarray, filename: str, file_bytes: bytes,
                          key: str = "", use_xor: bool = False) -> np.ndarray:
    """
    Embed a file into an image using Sequential LSB.
    The file is converted to Base64 and bundled with its filename.
    Format: [FILENAME]|[BASE64_DATA]
    """
    import base64
    b64_data = base64.b64encode(file_bytes).decode('utf-8')
    payload = f"{filename}|{b64_data}"
    return embed_sequential(image, payload, key, use_xor)


def extract_file_sequential(stego_image: np.ndarray, key: str = "",
                            use_xor: bool = False) -> tuple[str, bytes]:
    """
    Extract a hidden file from a stego image using Sequential LSB.
    Returns a tuple of (filename, file_bytes).
    """
    import base64
    extracted_text = extract_sequential(stego_image, key, use_xor)
    
    if '|' not in extracted_text:
        raise ValueError("Valid file header not found in extracted data.")
