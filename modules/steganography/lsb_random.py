...
def embed_file_random(image: np.ndarray, filename: str, file_bytes: bytes,
                      key: str, use_xor: bool = False) -> np.ndarray:
    """
    Embed a file into an image using Random LSB.
    ...
    return embed_random(image, payload, key, use_xor)


def extract_file_random(stego_image: np.ndarray, key: str,
                        use_xor: bool = False) -> tuple[str, bytes]:
    """
    Extract a hidden file from a stego image using Random LSB.
    Returns a tuple of (filename, file_bytes).
    """
    import base64
    extracted_text = extract_random(stego_image, key, use_xor)
