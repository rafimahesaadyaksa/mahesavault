...
def chi_square_attack(image: np.ndarray) -> tuple[list, list]:
    """
    Perform Chi-Square attack on the image to detect LSB steganography.
    It calculates the probability of embedding across chunks of the image.
    
    Args:
        image: Image as numpy array (BGR or Grayscale).
        
    Returns:
        Tuple of (x_axis_percentages, probabilities)
    """
    from scipy.stats import chi2
    
    # Convert to grayscale to simplify the test
    if len(image.shape) == 3:
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    else:
        gray = image
    ...
    # We will sample the image in chunks to see how the probability changes
    chunk_size = max(1, len(flat) // 100)
    
    probs = []
    x_axis = []
    
    for i in range(chunk_size, len(flat) + 1, chunk_size):
        chunk = flat[:i]
        ...
        if deg_freedom > 0:
            p = 1.0 - chi2.cdf(chi_sq, deg_freedom - 1)
            probs.append(p)
        else:
            probs.append(0)
            
        x_axis.append(i / len(flat) * 100)
        
    return x_axis, probs


def chi_square_figure(image: np.ndarray) -> plt.Figure:
    """
    Generate a figure showing the Chi-Square attack probability curve.
    
    Args:
        image: Image as numpy array.
        
    Returns:
        Matplotlib Figure showing the attack graph.
    """
    x, p = chi_square_attack(image)
    
    fig, ax = plt.subplots(figsize=(10, 5))
    fig.patch.set_facecolor('#0d0d0d')
    ax.set_facecolor('#111827')
    
    ax.plot(x, p, color='#ef4444', linewidth=2)
    ax.fill_between(x, p, alpha=0.15, color='#ef4444')
    
    # Threshold line at 0.5
    ax.axhline(y=0.5, color='#e2e8f0', linestyle='--', alpha=0.5, label='Threshold (0.5)')
    
    ax.set_title('Chi-Square Steganalysis Attack', color='#00f5ff', fontsize=14, fontfamily='monospace', fontweight='bold')
    ax.set_xlabel('Percentage of Image Sampled (%)', color='#e2e8f0')
    ax.set_ylabel('Probability of Embedding (1 = Certain)', color='#e2e8f0')
